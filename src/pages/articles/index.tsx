...
function ArticleListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<ArticleInterface[]>(
    ...
    () =>
      getArticles({
        relations: ['publisher', 'user_article_editor_idTouser', 'user_article_reporter_idTouser'],
      }),
  );
              ...
              <Thead>
                <Tr>
                  ...
                  {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>user_article_editor_idTouser</Th>
                  )}
                  {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>user_article_reporter_idTouser</Th>
                  )}
              ...
              <Tbody>
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/users/view/${record.user_article_editor_idTouser?.id}`}>
                          {record.user_article_editor_idTouser?.email}
                        </Link>
                      </Td>
                    )}
                    {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/users/view/${record.user_article_reporter_idTouser?.id}`}>
                          {record.user_article_reporter_idTouser?.email}
                        </Link>
                      </Td>
                    )}
